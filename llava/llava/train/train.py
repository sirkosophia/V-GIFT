    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0],
                            ...
                            image_path=self.list_data_dict[i].get("image", None),
                            raw_conversations=self.list_data_dict[i]["conversations"],)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        batch["sample_id"] = [ex.get("sample_id") for ex in instances]
        batch["image_path"] = [ex.get("image_path") for ex in instances]
        batch["raw_conversations"] = [ex.get("raw_conversations") for ex in instances]
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            if 'Qwen2' in model_args.model_name_or_path or 'qwen' in model_args.model_name_or_path.lower():
