...
def score_and_return_on_first_stage(model, tokens: torch.Tensor, lengths: torch.Tensor):
    ...
    if max_prompt_length > args.max_position_embeddings:
        raise ValueError(
            f"Length of prompt + tokens_to_generate longer than allowed {max_prompt_length} "
...
def generate_tokens_probs_and_return_on_first_stage(
    ...
    if max_sequence_length * batch_size > args.max_tokens_to_oom:
        raise ValueError(f"Too many tokens. {str(max_sequence_length*batch_size)} "
                         f"is greater than {str(args.max_tokens_to_oom)}")
...
def beam_search_and_return_on_first_stage(model, forward_step, tokens, lengths, beam_size, stop_token,
                                          num_return_gen, length_penalty, prevent_newline_after_colon=True):
    """run bsearch and return on first stage"""
    args = get_args()
    tokenizer = get_tokenizer()
