...
def beam_search_and_post_process(model,
    ...
    if mpu.is_pipeline_first_stage():
        ...
        tokens, prompts_plus_generations, prompts_plus_generations_segments = detokenize_generations(
            tokens, lengths, detokenize_segments)
...
def beam_search(model, forward_step, prompts=None, tokens_to_generate=0, beam_size=0, add_BOS=False,
                stop_token=50256, num_return_gen=1, length_penalty=1, prevent_newline_after_colon=False):
    """Run beam search."""
    # Make sure input params are avaialble to all ranks.
    values = [tokens_to_generate,
              beam_size,
