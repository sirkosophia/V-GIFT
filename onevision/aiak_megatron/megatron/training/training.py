...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True):
    ...
    def build_model():
        ...
        args.virtual_pipeline_model_parallel_size is not None:
            ...
            for i in range(args.virtual_pipeline_model_parallel_size):
                ...
                if use_dualpipev and args.num_nextn_predict_layers > 0:
                    if pre_process:
                        word_embeddings = this_model.shared_embedding_weight()
                model.append(this_model)
                if use_dualpipev and args.num_nextn_predict_layers > 0:
                    for this_model in model:
                        if this_model.mtp_layers is not None and len(this_model.mtp_layers) >= 1:
                            assert word_embeddings is not None, "DualPipeV requires a word embedding weight"
                            this_model.mtp_layers[0].embedding.word_embeddings.weight.data = word_embeddings.data
        else:
            pre_process = mpu.is_pipeline_first_stage()
            post_process = mpu.is_pipeline_last_stage()
            add_encoder = True
    ...
    # Before TE2.x: The model_module.bfloat16()/model_module.half() above will call the inplaceAdd commentMore actions
    ...
    if wrap_with_ddp:
        ...
        model = [DP(config=config,
                     ...
                     module=model_chunk,
                     # 1. Turn off bucketing for model_chunk 2 onwards, since communication for these
                     # model chunks is overlapped with compute anyway.
                     # 2. Turn off bucketing if we are on a pipeline stage that is not the first (since
                     # data-parallel communication on these stages is not on the critical path), or if
                     # disable_bucketing is True (e.g., we might not want to break up model parameters
                     # into buckets for model chunks after the first in the interleaved schedule).
                     disable_bucketing=args.enable_turn_off_bucketing and (
                        parallel_state.get_pipeline_model_parallel_rank() > 0
                        or model_chunk_idx > 0 
...
def enable_forward_pre_hook(model_chunks):
    """Enable forward pre hook for each model chunk"""
...
def disable_forward_pre_hook(model_chunks, param_sync=True):
    """Disable forward pre hook for each model chunk"""
    for model_chunk in model_chunks:
        assert isinstance(model_chunk, DDP)
...
def evaluate(forward_step_func,
    ...
    timers.log(['evaluate'], rank=0 if args.vpp_scheduler == 'dualpipev' else None)
...
def get_train_valid_test_num_samples():
    ...
    vp = args.num_virtual_stages_per_pipeline_rank
    if dualpipev:
        eval_iters *= vp
        test_iters *= vp
