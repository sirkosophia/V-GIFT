...
def default_embedding_ranks(pp_ranks, split_rank=None):
    ...
    global _DUALPIPEV
    if len(pp_ranks) == 1 or _DUALPIPEV:
        return [pp_ranks[0]]
...
def initialize_model_parallel(
    ...
    assert decoder_rank_generator.get_ranks("pp") == expert_decoder_rank_generator.get_ranks(
        ...
        f"but got {decoder_rank_generator.get_ranks('pp')} and {expert_decoder_rank_generator.get_ranks('pp')}")
...
def is_dualpipev_enabled():
    """Return True if dualpipev is enabled."""
    global _DUALPIPEV
    return _DUALPIPEV
...
def is_inside_encoder(rank=None) -> bool:
    ...
    if _PIPELINE_MODEL_PARALLEL_DECODER_START is not None and rank < _PIPELINE_MODEL_PARALLEL_DECODER_START:
        return True
    return False
...
def get_context_parallel_src_rank():
    ...
    assert (
        _CONTEXT_PARALLEL_GLOBAL_RANKS is not None
    ), "Context model parallel group is not initialized"
    return _CONTEXT_PARALLEL_GLOBAL_RANKS[0]
...
def get_pipeline_model_parallel_d2d_pair_rank():
    """Return the global rank that preceeds the caller in the pipeline"""
    assert _PIPELINE_GLOBAL_RANKS is not None, "Pipeline parallel group is not initialized"
    rank_in_pipeline = get_pipeline_model_parallel_rank()
    world_size = get_pipeline_model_parallel_world_size()
    assert world_size > rank_in_pipeline
    return _PIPELINE_GLOBAL_RANKS[(world_size - rank_in_pipeline - 1) % world_size]
