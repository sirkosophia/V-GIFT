...
def _allreduce_word_embedding_grads(model: List[torch.nn.Module], config: TransformerConfig):
    ...
    if (parallel_state.is_rank_in_embedding_group(ignore_virtual=True) and config.vpp_scheduler == 'dualpipev'):
        model_module0 = get_attr_wrapped_model(model[0], 'pre_process', return_model_obj=True)
        model_module1 = get_attr_wrapped_model(model[-1], 'pre_process', return_model_obj=True)

        if model_module0.share_embeddings_and_output_weights:
            weight0 = model_module0.shared_embedding_or_output_weight()
            weight1 = model_module1.shared_embedding_or_output_weight()
            assert weight0.main_grad is not None and weight1.main_grad is not None, "main_grad should be available."
            grad = weight0.main_grad + weight1.main_grad
            weight0.main_grad = grad
            weight1.main_grad = grad
        if getattr(model_module0, "share_mtp_embeddings_and_output_weights", False):
            assert not model_module0.share_embeddings_and_output_weights
            weight0 = model_module0.shared_embedding_weight()
            weight1 = model_module1.shared_embedding_weight()
            assert weight0.main_grad is not None and weight1.main_grad is not None, "main_grad should be available."
            grad = weight0.main_grad + weight1.main_grad
            weight0.main_grad = grad
            weight1.main_grad = grad
        ...
        # Sync the grad for the MTP embeddings and main model embedding weight.
        # FIXME: THIS IS NOT COMPATIBLE WITH `share_embeddings_and_output_weights` set to True.
        if getattr(model_module, "share_mtp_embeddings_and_output_weights", False):
            assert not model_module.share_embeddings_and_output_weights
