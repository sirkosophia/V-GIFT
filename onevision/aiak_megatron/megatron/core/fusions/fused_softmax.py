...
class FusedScaleMaskSoftmax(nn.Module):
    ...
    def forward(self, input: torch.Tensor, mask: Optional[torch.Tensor], attn_mask_type: Optional[AttnMaskType] = None):
        ...
        if self.is_kernel_available(mask, attn_mask_type, *input.size()):
            return self.forward_fused_softmax(input, mask, attn_mask_type)
        else:
            return self.forward_torch_softmax(input, mask, attn_mask_type)

    def is_kernel_available(self, mask, attn_mask_type, b, np, sq, sk):
        """Check whether kernel is available."""
        attn_batches = b * np
    ...
    def forward_fused_softmax(self, input, mask, attn_mask_type):
        """fused softmax"""
    ...
    def forward_torch_softmax(self, input, mask, attn_mask_type):
        """torch softmax"""
    ...
    def get_batch_per_block(sq, sk, b, np):
        """Get batch per block from sq, sk, b, np"""
        import scaled_masked_softmax_cuda
