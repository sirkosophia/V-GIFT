...
class ActivationCheckpoint(torch.autograd.Function):
    """Checkpoint a model or part of the model without output."""
    @staticmethod
...
class RecomputeManager:
    """Manager thr activation of a model."""
    def __init__(self):
        """init"""
    ...
    def checkpoint(self, run_function, distribute_saved_activations, *args):
        ...
        if distribute_saved_activations:
            raise RuntimeError(
                "distribute_saved_activations does not support "
            )
        ...
        outputs = ActivationCheckpoint.apply(run_function, self, *args)
