...
def get_forward_backward_func():
    ...
    if pipeline_model_parallel_size > 1:
        if parallel_state.get_virtual_pipeline_model_parallel_world_size() is not None:
            if not parallel_state.is_dualpipev_enabled():
...
def forward_backward_pipelining_with_interleaving(
    *,
    ...
    if config.combined_1f1b and not forward_only:
        ...
        forward_step_func = wrap_forward_func(config, forward_step_func, forward_only)
...
def forward_backward_pipelining_with_dualpipev(
    *,
    ...
    assert total_num_microbatches >= pipeline_parallel_size * 2, \
        (f"total_num_microbatches:{total_num_microbatches} not enough for dualpipev")
    config = get_model_config(model[0])
    set_streams()
    forward_step_func = wrap_forward_func(config, forward_step_func, forward_only)
    if config.overlap_p2p_comm and config.batch_p2p_comm:
    ...
    disable_grad_sync()
    is_first_rank = parallel_state.get_pipeline_model_parallel_rank() == 0
    is_last_rank = (parallel_state.get_pipeline_model_parallel_rank() ==
                    parallel_state.get_pipeline_model_parallel_world_size() - 1)
    #print(f"is_first_rank:{is_first_rank}, is_last_rank:{is_last_rank}")
    # Model chunk IDs with synchronized grads
    synchronized_model_chunks = set()
    current_f_chunk_id: List[int] = [0, 0]
    current_b_chunk_id: List[int] = [0, 0]
    current_send_f_chunk_id: List[int] = [0, 0]
    current_send_b_chunk_id: List[int] = [0, 0]
    current_recv_f_chunk_id: List[int] = [0, 0]
    current_recv_b_chunk_id: List[int] = [0, 0]

    global_comm_ops: List[torch.distributed.P2POp] = []
    global_shape_queue: List[Dict] = []
    to_free: List[torch.Tensor] = []

    input_tensors = [[] for _ in range(len(model))]
    if is_first_rank:
        for i in range(total_num_microbatches):
            input_tensors[0].append(None)
    output_tensors = [[] for _ in range(len(model))]
    total_num_tokens = torch.tensor(0, dtype=torch.int).cuda()
    ...
    if not forward_only:
        output_tensor_grads = [[] for _ in range(len(model))]
        input_tensor_grads = [[] for _ in range(len(model))]
    ...
    def combined_forward_backward_helper(
        ...
        if phase0 is not None:
            f_chunk_id = current_f_chunk_id[phase0]
            current_f_chunk_id[phase0] += 1
            f_context = VppContextManager(phase0)
            with f_context:
                input_tensor = input_tensors[phase0][f_chunk_id]
        ...
        b_output_tensor = None
        b_output_tensor_grad = None
        if phase1 is not None and not forward_only:
            b_chunk_id = current_b_chunk_id[phase1]
            current_b_chunk_id[phase1] += 1
            b_context = VppContextManager(phase1)
            with b_context:
                # launch grad synchronization (default)
                if config.grad_sync_func is None and b_chunk_id == total_num_microbatches - 1:
                    enable_grad_sync()
                    synchronized_model_chunks.add(phase1)

                if parallel_state.is_pipeline_last_stage():
                    output_tensor_grads[phase1].append(None)
                b_input_tensor = input_tensors[phase1][b_chunk_id]
                input_tensors[phase1][b_chunk_id] = None
                b_output_tensor = output_tensors[phase1][b_chunk_id]
                output_tensors[phase1][b_chunk_id] = None
                b_output_tensor_grad = output_tensor_grads[phase1][b_chunk_id]
                output_tensor_grads[phase1][b_chunk_id] = None

        output_tensor, num_tokens, input_tensor_grad = forward_backward_step(
            forward_step_func,
            data_iterator[phase0] if phase0 is not None else None,
            model[phase0] if phase0 is not None else None,
            total_num_microbatches,
            input_tensor,
            forward_data_store,
            model[phase1] if phase1 is not None and not forward_only else None,
            b_input_tensor,
            b_output_tensor,
            b_output_tensor_grad,
            config,
            ...
            is_first_microbatch=check_first_val_step(
                ...
                (
                    f_chunk_id == 0
                    if f_chunk_id is not None
                    else None
                ),
            ),
            current_microbatch=f_chunk_id,
        )

        # forward post process
        if f_chunk_id is not None:
            with f_context:
                if is_last_rank and phase0 == 0:
                    input_tensors[1].append(output_tensor.detach().requires_grad_())
                output_tensors[phase0].append(output_tensor)
                nonlocal total_num_tokens
                total_num_tokens += num_tokens.item()
                # If forward-only, no need to save tensors for a backward pass.
                if forward_only:
                #    # Release the tensor that have completed forward step.
                    input_tensors[phase0][f_chunk_id] = None
                    #output_tensors[phase0][f_chunk_id] = None

        # backward post process
        if b_chunk_id is not None and not forward_only:
            with b_context:
                # launch grad synchronization (custom grad sync)
                # Note: Asynchronous communication tends to slow down compute.
                # To reduce idling from mismatched microbatch times, we launch
                ...
                # pipeline-parallel group.
                if config.grad_sync_func is not None:
                    if b_chunk_id >= 0 and b_chunk_id == total_num_microbatches - 1:
                        enable_grad_sync()
                        config.grad_sync_func[phase1](
                            model[phase1].parameters()
                        )
                        synchronized_model_chunks.add(phase1)
                disable_grad_sync()
                if b_input_tensor is not None:
                    assert input_tensor_grad is not None
                    if is_last_rank and phase1 == 1:
                        output_tensor_grads[0].append(input_tensor_grad)
                    else:
                        input_tensor_grads[phase1].append(input_tensor_grad)
    ...
    def _recv_forward(phase: int, tensor_shape: Shape) -> None:
        if (is_first_rank and phase == 0) or (is_last_rank and phase == 1):
            return
        if config.variable_seq_lengths:
            # 将接收shape的操作加入队列
            nonlocal global_shape_queue
            global_shape_queue.append({
                "type": "recv_forward_shape",
                "phase": phase,
            })
        else:
            _recv_forward_real(phase, tensor_shape)

    def _recv_backward(phase: int, tensor_shape: Shape) -> None:
        if forward_only:
            return
        if (is_first_rank and phase == 1) or (is_last_rank and phase == 0):
            return
        if config.variable_seq_lengths:
            # 将接收shape的操作加入队列
            nonlocal global_shape_queue
            global_shape_queue.append({
                "type": "recv_backward_shape",
                "phase": phase,
            })
        else:
            _recv_backward_real(phase, tensor_shape)

    def _send_forward(phase: int) -> None:
        if (is_first_rank and phase == 1) or (is_last_rank and phase == 0):
            return

        chunk_id = current_send_f_chunk_id[phase]
        current_send_f_chunk_id[phase] += 1
        tensors = output_tensors[phase][chunk_id]
        if config.variable_seq_lengths:
            # 将发送shape的操作加入队列
            nonlocal global_shape_queue
            global_shape_queue.append({
                "type": "send_forward_shape",
                "phase": phase,
                "tensors": tensors
            })
        else:
            _send_forward_real(phase, tensors)

    def _send_backward(phase: int) -> None:
        if forward_only:
            return
        ...
        chunk_id = current_send_b_chunk_id[phase]
        current_send_b_chunk_id[phase] += 1
        tensors = input_tensor_grads[phase][chunk_id]
        input_tensor_grads[phase][chunk_id] = None
        if config.variable_seq_lengths:
            # 将发送shape的操作加入队列
            nonlocal global_shape_queue
            global_shape_queue.append({
                "type": "send_backward_shape",
                "phase": phase,
                "tensors": tensors
            })
        else:
            _send_backward_real(phase, tensors)

    def _recv_forward_real(phase: int, tensor_shape: Shape) -> None:
        #print(f"_recv_forward, phase:{phase}, pipeline_parallel_rank:{pipeline_parallel_rank}")
        current_recv_f_chunk_id[phase] += 1
        tensors, ops = p2p_communication.recv_forward_prev_or_next(
            prev_or_next="prev" if phase == 0 else "next", tensor_shape=tensor_shape, config=config
        )
        input_tensors[phase].append(tensors)
        nonlocal global_comm_ops
        global_comm_ops.extend(ops)

    def _send_forward_real(phase: int, tensors: torch.Tensor) -> None:
        ops = p2p_communication.send_forward_prev_or_next(
            tensors, prev_or_next="next" if phase == 0 else "prev", config=config
        )
        nonlocal global_comm_ops
        global_comm_ops.extend(ops)
        if forward_only:
            chunk_id = current_send_f_chunk_id[phase] - 1
            output_tensors[phase][chunk_id] = None
            to_free.append(tensors)

        #deallocate_output_tensor(tensors, config.deallocate_pipeline_outputs)

    def _recv_backward_real(phase: int, tensor_shape: Shape) -> None:
        #print(f"_recv_backward, phase:{phase}, pipeline_parallel_rank:{pipeline_parallel_rank}")
        current_recv_b_chunk_id[phase] += 1
        tensors, ops = p2p_communication.recv_backward_prev_or_next(
            tensor_shape, prev_or_next="next" if phase == 0 else "prev", config=config
        )
        nonlocal global_comm_ops
        global_comm_ops.extend(ops)
        output_tensor_grads[phase].append(tensors)
        #print(f"finish _recv_backward, phase:{phase}, pipeline_parallel_rank:{pipeline_parallel_rank}")

    def _send_backward_real(phase: int, tensors: torch.Tensor) -> None:
        ops = p2p_communication.send_backward_prev_or_next(
            tensors, prev_or_next="prev" if phase == 0 else "next", config=config
        )
        nonlocal global_comm_ops
        global_comm_ops.extend(ops)

    def _forward_chunk(phase: int, tensor_shape: Shape, recv: bool = True, send: bool = True) -> None:
        if recv:
            _recv_forward(phase, tensor_shape)
    ...
    def _backward_chunk(phase: int,
                        tensor_shape: Shape,
                        enable_zb: bool = False,
                        recv: bool = True,
                        send: bool = True) -> None:
        if recv:
            _recv_backward(phase, tensor_shape)
        commit_and_wait_comm()
        ...
        if send:
            _send_backward(phase)

    def _forward_backward_chunk(phase0: int, phase1: int, tensor_shape: Shape, recv0: bool = True) -> None:
        if recv0:
            _recv_forward(phase0, tensor_shape)
        _recv_backward(phase1, tensor_shape)
        commit_and_wait_comm()
        forward_backward_helper_wrapper(
            phase0=phase0,
            phase1=phase1,
        ...
        _send_forward(phase0)
        _send_backward(phase1)

    def free_tensors() -> None:
        nonlocal to_free
        for tensor in to_free:
            assert tensor._base is None, \
                f"pipeline stage should not return view tensors {torch.distributed.get_rank(), tensor.shape}"
            tensor.data = torch.Tensor()
        to_free.clear()

    def process_shape_queue() -> None:
        nonlocal global_shape_queue
        ops = []
        func_queue = []

        for q in global_shape_queue:
            q_type = q["type"]
            phase = q["phase"]

            # 处理接收形状的操作
            if q_type in ["recv_forward_shape", "recv_backward_shape"]:
                recv_shape_tensor = torch.empty((3), device=torch.cuda.current_device(), dtype=torch.int64)

                # 确定发送方的rank
                if q_type == "recv_forward_shape":
                    src_rank = get_pipeline_model_parallel_prev_rank() \
                        if phase == 0 else get_pipeline_model_parallel_next_rank()
                    func_type = "recv_forward"
                else:  # recv_backward_shape
                    src_rank = get_pipeline_model_parallel_next_rank() \
                        if phase == 0 else get_pipeline_model_parallel_prev_rank()
                    func_type = "recv_backward"
                ...
                func_queue.append({
                    "type": func_type,
                    "phase": phase,
                    "recv_shape_tensor": recv_shape_tensor
                })

            # 处理发送形状的操作
            elif q_type in ["send_forward_shape", "send_backward_shape"]:
                tensors = q["tensors"]
                send_shape_tensor = torch.tensor(
                    tensors.size(), device=torch.cuda.current_device(), dtype=torch.int64
                )

                # 确定接收方的rank
                if q_type == "send_forward_shape":
                    dst_rank = get_pipeline_model_parallel_next_rank() \
                        if phase == 0 else get_pipeline_model_parallel_prev_rank()
                    func_type = "send_forward"
                else:  # send_backward_shape
                    dst_rank = get_pipeline_model_parallel_prev_rank() \
                        if phase == 0 else get_pipeline_model_parallel_next_rank()
                    func_type = "send_backward"
        ...
        # 处理通信结果
        for f in func_queue:
            if f["type"] == "recv_forward":
                _recv_forward_real(f["phase"], f["recv_shape_tensor"].tolist())
            elif f["type"] == "recv_backward":
                _recv_backward_real(f["phase"], f["recv_shape_tensor"].tolist())
            elif f["type"] == "send_forward":
                _send_forward_real(f["phase"], f["tensors"])
            elif f["type"] == "send_backward":
                _send_backward_real(f["phase"], f["tensors"])

        # 清空队列
        global_shape_queue.clear()

    def commit_and_wait_comm() -> None:
        process_shape_queue()
        nonlocal global_comm_ops
        if not global_comm_ops:
            return
        reqs = torch.distributed.batch_isend_irecv(global_comm_ops)
        for req in reqs:
            req.wait()
        global_comm_ops.clear()
        free_tensors()
    ...
    for i in range(step_1):
        _forward_chunk(0, tensor_shape)

    # Step 2: nF0F1
    step_2 = rank + 1
    _recv_forward(0, tensor_shape)
    for i in range(step_2):
        _forward_chunk(0, tensor_shape, recv=False, send=False)
        _recv_forward(0, tensor_shape)
        _forward_chunk(1, tensor_shape, send=(not is_last_rank) or (i < step_2 - 1))
        _send_forward(0)

    # Step 3: nB1W1F1 (Use zero bubble)
    step_3 = num_ranks - rank - 1
    for i in range(step_3):
        _backward_chunk(1, tensor_shape, enable_zb=True)
        _recv_forward(1, tensor_shape)
        #_weight_chunk()
        _forward_chunk(1, tensor_shape, recv=False)

    # Step 4 (Main step): nF0B1F1B0
    step_4 = total_num_microbatches - num_ranks * 2 + rank + 1
    for i in range(step_4):
        if i == 0:
            if is_last_rank:
                ...
                _forward_chunk(0, tensor_shape, recv=False, send=False)
                _send_forward(1)
                _backward_chunk(1, tensor_shape, send=False)
                _send_forward(0)
                _send_backward(1)
            else:
                _forward_backward_chunk(0, 1, tensor_shape, recv0=False)
        else:
            _forward_backward_chunk(0, 1, tensor_shape)
        _forward_backward_chunk(1, 0, tensor_shape)

    # Step 5: nB1F1B0
    step_5 = num_ranks - rank - 1
    for i in range(step_5):
        _backward_chunk(1, tensor_shape)
        _forward_backward_chunk(1, 0, tensor_shape)
    ...
    for i in range(step_6):
        if i == step_6 // 2 and rank % 2 == 1:
            enable_zb = True
        _backward_chunk(1, tensor_shape, enable_zb=enable_zb)
        if i == step_6 // 2 and rank % 2 == 0:
            enable_zb = True
        _backward_chunk(0, tensor_shape, enable_zb=enable_zb)

    # Step 7: nWB0 (Use zero bubble)
    step_7 = num_ranks - rank - 1
    for i in range(step_7):
        #_weight_chunk()
        _backward_chunk(0, tensor_shape, enable_zb=True)
