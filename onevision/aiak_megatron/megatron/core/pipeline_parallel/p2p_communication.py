...
def _create_shape_recv_prev(tensor_send_next, tensor_send_prev, recv_prev, recv_next):
    """ Create tensors used in shape communication """
    ...
    return recv_prev_shape_tensor, recv_next_shape_tensor, send_prev_shape_tensor, send_next_shape_tensor
...
def _communicate_shapes(tensor_send_next, tensor_send_prev, recv_prev, recv_next, config):
    ...
    if config.use_ring_exchange_p2p:
        recv_prev_shape_tensor, recv_next_shape_tensor, send_prev_shape_tensor, send_next_shape_tensor \
                = _create_shape_recv_prev(tensor_send_next, tensor_send_prev, recv_prev, recv_next)
        ...
        recv_prev_shape_tensor_list = []
        recv_next_shape_tensor_list = []
        ...
        for group, nr, pr in zip(pp_group, next_rank, prev_rank):
            ...
            if recv_prev_shape_tensor is not None:
                recv_prev_shape_tensor_list.append(recv_prev_shape_tensor)
            if recv_next_shape_tensor is not None:
                recv_next_shape_tensor_list.append(recv_next_shape_tensor)
    ...
    def _handle_shape_list(x):
        if len(x) == 0:
            return None
        else:
            assert all(xx.tolist() == x[0].tolist() for xx in x)
            return x[0]

    recv_prev_shape_tensor = _handle_shape_list(recv_prev_shape_tensor_list)
    recv_next_shape_tensor = _handle_shape_list(recv_next_shape_tensor_list)
...
def recv_forward_prev_or_next(tensor_shape: Shape, prev_or_next: str, config: ModelParallelConfig)\
        -> Tuple[torch.Tensor, List[torch.distributed.P2POp]]:
    """Receive tensor from next rank in pipeline (backward receive).
    ...
    if config.timers is not None:
        config.timers('forward-recv', log_level=2).start()
    tensor_recv_prev, tensor_recv_next, ops = _communicate_dualpipev(
        tensor_send_next=None,
        tensor_send_prev=None,
        recv_prev=True if prev_or_next == "prev" else False,
        recv_next=True if prev_or_next == "next" else False,
        tensor_shape=tensor_shape,
        config=config
    )
    if config.timers is not None:
        config.timers('forward-recv').stop()
    return tensor_recv_prev if prev_or_next == "prev" else tensor_recv_next, ops

def recv_backward_prev_or_next(tensor_shape: Shape, prev_or_next: str, config: ModelParallelConfig)\
        -> Tuple[torch.Tensor, List[torch.distributed.P2POp]]:
    """Receive tensor from next rank in pipeline (backward receive).
...
def send_forward_prev_or_next(output_tensor: torch.Tensor, prev_or_next: str, config: ModelParallelConfig) \
    ...
    if config.timers is not None:
        config.timers('forward-send', log_level=2).start()
    _, _, ops = _communicate_dualpipev(
        tensor_send_next=output_tensor if prev_or_next == "next" else None,
        tensor_send_prev=output_tensor if prev_or_next == "prev"  else None,
        recv_prev=False,
        recv_next=False,
        tensor_shape=None,
        config=config
...
def send_backward_prev_or_next(input_tensor_grad: torch.Tensor, prev_or_next: str, config: ModelParallelConfig) \
        -> List[torch.distributed.P2POp]:
    """Send tensor to next rank in pipeline (forward send).
    ...
    if config.timers is not None:
        config.timers('backward-send', log_level=2).start()
    _, _, ops = _communicate_dualpipev(
        tensor_send_prev=input_tensor_grad if prev_or_next == "prev" else None,
        tensor_send_next=input_tensor_grad if prev_or_next == "next" else None,
        recv_prev=False,
        recv_next=False,
        tensor_shape=None,
        config=config
...
def _batched_p2p_ops_dualpipev(
    *,
    tensor_send_prev: Optional[torch.Tensor],
    tensor_recv_prev: Optional[torch.Tensor],
    tensor_send_next: Optional[torch.Tensor],
...
def _communicate_dualpipev(
    *,
    ...
    tensor_shape: Shape,
    config: ModelParallelConfig,
) -> Tuple[torch.Tensor, torch.Tensor, List[torch.distributed.P2POp]]:
    """Communicate tensors between stages. Used as helper method in other
    communication methods that are used in megatron/schedules.py.
    ...
    p2p_func = _batched_p2p_ops_dualpipev
