...
class ModelParallelConfig:
    ...
    vpp_scheduler: str = None
    """By default, the original 1F1B scheduling method is used. When selecting DualPipeV, '
    ...
    # AIAK Added
    ###################
    custom_pipeline_layers: Optional[List[int]] = None
    """Custom layers for each stage"""

    custom_pipeline_recompute_layers: Optional[List[int]] = None
    """Custom recomputation layers for each stage"""
    
    # TODO: not supported yet
    flash_attn_checkpoint: bool = False
    """Whether to enable FlashAttention checkpointing without redundancy fa fwd"""

    single_all_to_all: bool = False
    """Single all-to-all communication"""

    dense_mlp_activation_func_recompute: bool = False
    
    layernorm_recompute: bool = False

    moe_mlp_activation_func_recompute: bool = False

    mla_recompute: bool = False
    ...
    def __post_init__(self):
        ...
        if (self.split_bw and 
            not (self.combined_1f1b and self.combined_1f1b_recipe == "ep_a2a") and 
            not (self.vpp_scheduler == "dualpipev")):
            raise ValueError(
                "Spliting dgrad and wgrad only works when setting --combined-1f1b and "
                "--combined-1f1b-recipe ep_a2a or when setting dualpipev"
            )
