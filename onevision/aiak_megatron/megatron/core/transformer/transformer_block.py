...
def get_num_layers_to_build(config: TransformerConfig) -> int:
    ...
    if config.pipeline_model_parallel_size > 1 and config.custom_pipeline_layers is not None:
        # uneven pipeline (AIAK Version)
        assert not config.account_for_embedding_in_pipeline_split or config.account_for_loss_in_pipeline_split, \
            "Does not support standalone embedding stage and standalone loss stage with uneven pp"

        num_layers_per_pipeline_rank = config.custom_pipeline_layers[parallel_state.get_pipeline_model_parallel_rank()]
        ...
        if (
            pp_rank == parallel_state.get_pipeline_model_parallel_world_size() - 1
    ...
    if parallel_state.get_virtual_pipeline_model_parallel_world_size() is not None:
        ...
        # if num_layers_per_pipeline_rank = 5, vp_size = 2, then q = 2, r = 1
        # we will get vp_rank_0 = 2 vp_rank_1 = 3
...
class TransformerBlock(MegatronModule):
    ...
    def _build_layers(self):
        ...
        def build_layer(layer_spec, layer_number):
            global_layer_number = layer_number + get_transformer_layer_offset(self.config)  # 1-based index
        ...
        if self.submodules.layer_norm and self.post_process and self.post_layer_norm:
            ...
            def remove_extra_states_check(self, incompatible_keys):
                """
                Temporarily remove final_layernorm._extra_state as a missing key
                when loading older TransformerEngine checkpoints.
                """
                for key in incompatible_keys.missing_keys:
                    if "final_layernorm._extra_state" in key:
