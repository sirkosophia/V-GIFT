...
class TransformerConfig(ModelParallelConfig):
    ...
    """enable fp8 format comm in dispatch under deepep."""
    ...
    enable_mem_monitor: bool = False
    """ Whether to enable memory monitor. """

    print_mem_monitor_interval: int = 1000
    """ Interval to print memory monitor. """

    mem_monitor_log: str = None
    """ Path to log memory monitor. """

    mem_monitor_force_print_token_threshold: int = 100000000
    ...
    def __post_init__(self):
        ...
        if self.custom_pipeline_recompute_layers is not None:
            self.recompute_num_layers = self.custom_pipeline_recompute_layers[get_pipeline_model_parallel_rank()]
        ...
        if (
            ...
            if self.num_layers_in_first_pipeline_stage is not None:
                ...
                if self.virtual_pipeline_model_parallel_size is not None:
                    if self.vpp_scheduler != "dualpipev":
                        if self.num_layers_in_first_pipeline_stage % self.virtual_pipeline_model_parallel_size != 0:
                            raise ValueError(
            ...
            if self.num_layers_in_last_pipeline_stage is not None:
                ...
                if self.virtual_pipeline_model_parallel_size is not None:
                    if self.vpp_scheduler != "dualpipev":
                        if self.num_layers_in_last_pipeline_stage % self.virtual_pipeline_model_parallel_size != 0:
                            raise ValueError(
        ...
        if self.apply_rope_fusion:
            ...
            if fused_apply_rotary_pos_emb is None and fused_apply_rotary_pos_emb_thd is None:
                raise ValueError("apply_rope_fusion is not available. Please install TE >= 1.4 or Apex.")
        ...
        if self.cp_comm_type is not None:
            if isinstance(self.cp_comm_type, list):
                ...
                assert isinstance(self.cp_comm_type, str), "Unsupported communication type for context parallelism!"
    ...
    def _check_moe_configs(self):
        ...
        if self.enable_fp8_comm:
            if not self.moe_enable_deepep:
                raise ValueError("fp8 comm is only supported under deepep mode.")
        ...
        if (
            ...
            raise ValueError("alltoall_seq dispatcher not support different TP size for MoE and Dense layer.")
        ...
        if self.moe_token_dispatcher_type in ['allgather', 'alltoall_seq']:
            if self.variable_seq_lengths is True:
                ...
                warnings.warn(
                    f"Token dispatcher type replaced to alltoall due to variable seqlen.\n"
