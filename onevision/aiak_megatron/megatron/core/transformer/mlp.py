...
from megatron.core.tensor_parallel.random import RecomputeManager
...
class ActivationFuncModule(MegatronModule):
    """ActivationFuncModule"""
    ...
    def __init__(self, config, submodules):
        ...
        self.activation_func_fp8_input_store = config.activation_func_fp8_input_store
        self.fused_activation_type = "gelu"

        if self.bias_activation_fusion:
            if self.activation_func == F.gelu:
                if config.gated_linear_unit:
                    self.bias_activation_func_impl = bias_geglu_impl
                    self.fused_activation_type = "geglu"
                else:
                    assert config.add_bias_linear is True
                    self.bias_activation_func_impl = bias_gelu_impl
            elif self.activation_func == F.silu and config.gated_linear_unit:
                self.bias_activation_func_impl = bias_swiglu_impl
                self.fused_activation_type = "swiglu"
            else:
            ...
            if config.gated_linear_unit:
                ...
                self.bias_activation_func_impl = glu
            else:
                self.bias_activation_func_impl = self.activation_func

        if submodules.bias_activation_func_impl is not None:
            self.bias_activation_func_impl = \
                    build_module(submodules.bias_activation_func_impl[self.fused_activation_type])

    def forward(self, intermediate, bias):
        """Applies the activation function to the incoming data."""
        if self.bias_activation_fusion:
            if self.activation_func == F.gelu:
                intermediate = self.bias_activation_func_impl(intermediate, bias)
            elif self.activation_func == F.silu:
                intermediate = self.bias_activation_func_impl(
                    intermediate,
                    bias,
                    self.activation_func_fp8_input_store,
                )
        else:
            if bias is not None:
                intermediate = intermediate + bias
            intermediate = self.bias_activation_func_impl(intermediate)
        return intermediate
...
class MLP(MegatronModule):
    ...
    def activation_func_recompute_forward(self, hidden_states):
        """activation_func_recompute forward """
        #print ("activation_func_recompute_forward step1")
        # [s, b, 4 * h/p]
        intermediate_parallel, bias_parallel = self.linear_fc1(hidden_states)

        # activation function
        self.recompute_manager = RecomputeManager()
        intermediate_parallel = self.recompute_manager.checkpoint(self.activation_func,
                                                                  False,
                                                                  intermediate_parallel,
                                                                  bias_parallel)
        # [s, b, h]
        output, output_bias = self.linear_fc2(intermediate_parallel)
        self.recompute_manager.discard_output()
        if output.requires_grad:
            output.register_hook(self.recompute_manager.recompute)

        return output, output_bias

    def forward(self, hidden_states):
        """ forward """
        if self.config.dense_mlp_activation_func_recompute:
            return self.activation_func_recompute_forward(hidden_states)
