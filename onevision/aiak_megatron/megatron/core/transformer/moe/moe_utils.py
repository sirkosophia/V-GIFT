...
def group_limited_topk(
    ...
    # FIX: Align with deepseek v3 paper
    group_scores = scores.view(num_tokens, num_groups, -1).topk(math.ceil(topk / group_topk), dim=-1)[0].sum(dim=-1)
...
def topk_softmax_with_capacity(
    ...
    if score_function == "softmax":
        if use_pre_softmax:
            ...
            scores = scores.type_as(logits) if ori_dtype is None else scores.to(ori_dtype)
            probs, top_indices = compute_topk(scores, topk, num_groups, group_topk)
        else:
            scores, top_indices = compute_topk(logits, topk, num_groups, group_topk)
            probs = torch.softmax(scores, dim=-1, dtype=torch.float32)
            probs = probs.type_as(logits) if ori_dtype is None else probs.to(ori_dtype)
    ...
    topk_masked_gates = torch.zeros_like(logits.to(ori_dtype) if ori_dtype is not None else logits) \
            .scatter(1, top_indices, probs)
