...
class TEGroupedMLP(MegatronModule):
    ...
    def bias_swiglu_impl_checkpoint(self, intermediate_parallel,
                        bias_parallel,):
        """bias_swiglu_impl_checkpoint"""
        return bias_swiglu_impl(intermediate_parallel, bias_parallel, self.config.activation_func_fp8_input_store,)
    ...
    def forward(
        ...
        if self.config.moe_mlp_activation_func_recompute:
            self.activation_func_recompute_manager = RecomputeManager()
        ...
        if self.config.bias_activation_fusion:
            if self.activation_func == F.gelu:
                if self.config.gated_linear_unit:
                    if self.config.moe_mlp_activation_func_recompute:
                       intermediate_parallel = self.activation_func_recompute_manager.checkpoint(
                                                    bias_geglu_impl,
                                                    False,
                    ...
                    if self.config.moe_mlp_activation_func_recompute:
                       intermediate_parallel = self.activation_func_recompute_manager.checkpoint(bias_gelu_impl,
                                                                                                 False,
                                                                                                 intermediate_parallel,
                                                                                                 bias_parallel)
                    else:
                ...
                if self.config.moe_mlp_activation_func_recompute:
                    intermediate_parallel = self.activation_func_recompute_manager.checkpoint(
                                                     self.bias_swiglu_impl_checkpoint,
                                                     False,
                                                     intermediate_parallel,
                                                     bias_parallel)
                else:
        ...
        if self.config.moe_mlp_activation_func_recompute:
            self.activation_func_recompute_manager.discard_output()
            if output.requires_grad:
                output.register_hook(self.activation_func_recompute_manager.recompute)
