...
class MoEAlltoAllTokenDispatcher(MoETokenDispatcher):
    ...
    def per_batch_state_context(self, state: MoEAlltoAllPerBatchState):
        """ Context manager to save and restore the per-batch state."""
    ...
    def meta_prepare(
        ...
        """ meta prepare for AlltoAll communication and token permutation."""
        self.hidden_shape = hidden_states.shape
        self.probs = probs
    ...
    def sync_meta_dtoh(self, point):
        """ Synchronize the CUDA stream and transfer metadata from GPU to CPU."""
    ...
    def dispatch_postprocess(self, global_input_tokens):
        """ Postprocess the tokens after AlltoAll communication."""
        if self.shared_experts is not None:
    ...
    def token_permutation(
        ...
        permutated_local_input_tokens = self.dispatch_preprocess(hidden_states)
    ...
    def combine_preprocess(self, hidden_states):
        """ Preprocess the hidden states after AlltoAll communication."""
    ...
    def combine_all_to_all(self, hidden_states):
        """ Combine the hidden states after AlltoAll communication."""
        # Perform expert parallel AlltoAll communication
    ...
    def combine_postprocess(self, permutated_local_input_tokens):
        """ Postprocess the hidden states after AlltoAll communication."""
        if self.shared_experts is not None:
...
class MoEFlexTokenDispatcher(MoETokenDispatcher):
    ...
    def collect_per_batch_state(self, state: MoEFlexPerBatchState):
        """ Collect per batch state for Flex token dispatcher."""
        pass

    def apply_per_batch_state(self, state: MoEFlexPerBatchState):
        """ Apply per batch state for Flex token dispatcher."""
        pass
    ...
    def set_shared_experts(self, shared_experts):
        """ Set the shared experts for Flex token dispatcher."""
    ...
    def dispatch_preprocess(self, hidden_states: torch.Tensor, routing_map: torch.Tensor, probs: torch.Tensor):
        """ Preprocess the hidden states for AlltoAll communication."""
        self.hidden_shape = hidden_states.shape
        hidden_states = hidden_states.view(-1, self.hidden_shape[-1])
    ...
    def token_unpermutation(
        ...
        """ Unpermutation the hidden states after AlltoAll communication."""
