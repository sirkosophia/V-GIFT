...
"""abstract_model_inference_wrapper"""
import abc
import math
...
class AbstractModelInferenceWrapper(abc.ABC):
    ...
    def get_batch_for_context_window(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        This function gets called iteratively in the inference loop . It can be used to extract
        relevant input from the prompt tokens, attention mask etc. required for each step in inference.
    ...
    def forward_pass_without_pipeline_parallel(
        ...
        Runs a very simple forward pass for model. Used  in the case of models without any parallelism or
    ...
    def forward_pass_with_pipeline_parallel_small_input_batch(
        ...
        If a model is pipeline parallel, yet, the input global batch is very small, we compute a foward pass
        on the entire global batch, rather than splitting it up into micro batches and
        doing something more complex as in the forward_pass_with_pipeline_parallel_large_input_batch method
