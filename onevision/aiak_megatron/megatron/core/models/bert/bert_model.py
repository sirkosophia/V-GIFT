...
class BertModel(LanguageModule):
    ...
    def __init__(
        ...
        ), "Bert currently does not support flash attention. \
            Please set env variable NVTE_FLASH_ATTN=0 or set NVTE_ALLOW_NONDETERMINISTIC_ALGO=0"
    ...
    def _sanity_check_attention_and_get_attn_mask_dimension(self) -> str:
        ...
        Default if you dont set any NVTE_ATTN flag will it will just use the fused path for
        transformer engine version >= 1.7 and unfused path for other
        ...
        if (
            ...
            assert attention_backend in [
                AttnBackend.local,
                AttnBackend.auto,
            ], (f'Expected AttnBackend to be local or auto while using mcore self attention, '
                f'but found {attention_backend}. Set --attn-backend to local or '
                f'dont use MCore SelfAttention submodule in layer specs')
            ...
            if is_te_min_version("1.10.0"):
                ...
                if attn_mask_type != AttnMaskType.padding:
                    warnings.warn(
                        f'For TE versions >= 1.10 , flash/fused/unfused support padding mask. '
                        f'Setting attention mask from {attn_mask_type} to padding'
                    )
                    self.transformer_layer_spec.submodules.self_attention.params[
                        'attn_mask_type'
                    ] = AttnMaskType.padding
            # For 1.7 >= TE < 1.10 flash and fused path use padding mask with b11s and unfused path
            # uses arbitrary mask with b1ss
                ...
                if attention_backend in [AttnBackend.flash, AttnBackend.fused, AttnBackend.auto]:
                    ...
                    if attn_mask_type != AttnMaskType.arbitrary:
                        warnings.warn(
                            f'For TE versions >= 1.7 but < 1.10 , unfused path supports only arbitrary mask. '
                            f'Setting attention mask from {attn_mask_type} to arbitray'
                        )
                        self.transformer_layer_spec.submodules.self_attention.params[
                            'attn_mask_type'
            ...
            # For TE < 1.7 we only support unfused attention with b1ss and padding mask
                ...
                assert not (attention_backend in [AttnBackend.flash, AttnBackend.fused]), (
                    ...
                    "< 1.7. Set --attention-backend to unfused or leave it to be default (auto) or "
                    "upgrade transformer engine >= 1.7"
                )
