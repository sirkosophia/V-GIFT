...
class LanguageModule(MegatronModule):
    ...
    def _set_attention_backend(self):
        ...
        Transformer engine works based on optout. By default all three attention backend flags are set to 1.
        So if the user choses a particular attention backend we set the other two to 0.
        If the user choses local, we set all 3 TE env variables to 0.
        """
        ...
        def check_and_set_env_variable(
            ...
            assert current_value is None or current_value == str(expected_value), (
                f'{env_variable_name} set to {current_value}, but expected {expected_value} for attention backend '
                f'type {attn_type.name}. unset NVTE_FLASH_ATTN, NVTE_FUSED_ATTN and NVTE_UNFUSED_ATTN. '
                f'Use the --attention-backend argument if you want to choose between (flash/fused/unfused/auto/local). '
                f'Default is auto.')
