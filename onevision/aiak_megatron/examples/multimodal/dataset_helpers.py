...
# Based on https://github.com/hiyouga/LLaMA-Factory/blob/641d0dab08d96a93c34657742213d8994d9ed476/src/
# llamafactory/data/processors/processor_utils.py#L19
...
# Based on https://github.com/hiyouga/LLaMA-Factory/blob/641d0dab08d96a93c34657742213d8994d9ed476/src/
# llamafactory/data/processors/processor_utils.py#L27
# Copyright (c) 2024 LLaMA-Factory. Apache license 2.0.
def greedy_knapsack(item_sizes: List[int], samples: List, max_capacity: int) -> List:
    ...
    if sorted_item_sizes[-1] > max_capacity:
        raise ValueError(f"knapsack: A sample is larger {sorted_item_sizes[-1]} "
...
class TaskEncoder(DefaultTaskEncoder[OCRSample, OCRSample, ImageTaskBatchPacked, dict]):
    ...
    def encode_sample(self, sample: Union[CaptioningSample, OCRSample, VQASample, SimilarityInterleavedSample]):
        ...
        if isinstance(sample, OCRSample):
            ...
            is_llava_training = sample.__subflavors__["is_llava_training"] \
                if "is_llava_training" in sample.__subflavors__ else False
    ...
    def encode_captioning(self, sample: CaptioningSample):
        ...
        imgs = get_visual_transform(
            sample.image, self.img_h, self.img_w, self.args.use_tiling,
            self.args.max_num_tiles, self.args.use_thumbnail, augment,
        ...
        for idx, sample in enumerate(samples.samples):
            try:
                ...
                if current_length + encoded_sample.total_len > self.packing_seq_length:
                    print(f"Encoding list of samples: stopped at {idx+1} samples to stick to "
                          f"{self.packing_seq_length}. Last sample key: {sample.__key__}")
                    break
                else:
    ...
    def encode_llava_sft(self,
        ...
        if type(sample).__name__ == "OfflineTargetAspectRatioSample":
            ...
            assert target_aspect_ratio is not None, \
        ...
        error_msg = (
            f"Found {number_image_token} image tokens for len({num_tiles}) = {len(num_tiles)} "
            f"image tiles in {conversation}.")
    ...
    def target_has_trainable_tokens(self, input_ids, num_tiles, target):
        """Check whether the target contains any trainable tokens."""
    ...
    def encode_ocr_ref_prompt(self, sample: OCRSample) -> ImageTaskSample:
        ...
        if len(region) == 4:
            ...
            region = (f"<quad>({region[0]},{region[1]}),({region[2]},{region[3]}),"
                      f"({region[4]},{region[5]}),({region[6]},{region[7]})</quad>")
    ...
    def bbox_coord_to_label(self, text, bbox):
        ...
        if len(bbox) == 4:
            ...
            label_str = (f"<ref>{text}</ref><quad>({bbox[0]},{bbox[1]}),({bbox[2]},{bbox[3]}),"
                         f"({bbox[4]},{bbox[5]}),({bbox[6]},{bbox[7]})</quad>")
