...
class MMMUDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # The following downloads the MMMU dataset from HuggingFace and
        # uses the API from the MMMU github repo to run MMMU evaluation.
        all_mmmu_datasets = []
    ...
    def __getitem__(self, idx):
        ...
        if self._prompt_style == "single_image":
            ...
            sample["final_input_prompt"] = (" ".join(
                [f'<image {i + 1}><image>' for i in range(len(img_indices))]) + "\n" + sample["final_input_prompt"])
...
def get_evaluation_dataset(
    ...
    if task == "TextVQA":
        ...
        # - prompt_style="vlmevalkit" is similar to https://github.com/open-compass/VLMEvalKit/blob/
        # 5d3cebcf18ef4bfbadc3bd3ef80bdc7aad2c6557/vlmeval/vlm/internvl_chat.py#L499
        dataset = MMMUDataset(
            input_image_path,
            num_samples_per_partition,
            num_partitions,
