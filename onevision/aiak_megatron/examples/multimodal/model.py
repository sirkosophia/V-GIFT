...
def model_provider(
    ...
    assert args.encoder_pipeline_model_parallel_size <= 1, \
        "LLaVA does not support pp>1 for encoder on it's own pipeline rank"
    ...
    if args.decoder_seq_length > args.max_position_embeddings:
        ...
        warnings.warn(
            f"Expanded max_position_embeddings to {args.max_position_embeddings} to accommodate the maximum "
            f"language model sequence length"
        )
    ...
    if vision_model_type.startswith("huggingface"):
        assert args.encoder_tensor_model_parallel_size < 2, \
            "Huggingface vision encoders do not support --encoder-tensor-model-parallel-size > 1"
        assert args.encoder_pipeline_model_parallel_size == 0, \
            "Huggingface vision encoders do not support --encoder-pipeline-model-parallel-size > 0"
        ...
        assert args.vision_huggingface_model_name_or_path is not None, \
            "Providing --vision-huggingface-model-name-or-path is necessary when using huggingface vision model"
    ...
    if base_config.language_model_type.startswith("huggingface"):
        ...
        assert not args.sequence_parallel, "Huggingface models do not support --sequence-parallel"
        assert args.context_parallel_size < 2, "Huggingface models do not support --context-parallel-size > 1"
        assert args.language_huggingface_model_name_or_path is not None, \
            "Providing --language-huggingface-model-name-or-path is necessary when using huggingface language model"
