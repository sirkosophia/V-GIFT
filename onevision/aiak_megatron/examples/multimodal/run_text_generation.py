...
def generate_samples(model, config: EvaluationConfig, print_output):
    ...
    for idx, (imgs, num_tiles, sample_id, question, answers, metadata) in enumerate(dataloader):
        ...
        if not args.use_mcore_inference:
            forward_step = partial(VLMForwardStep, num_img_embeddings_per_tile,
                                   imgs, num_tiles, args.decoder_seq_length)
...
class VLMForwardStep(ForwardStep):
    ...
    def __init__(
        ...
        if pp_rank > 0:
            self._recv_only_vision_embeds = parallel_state.is_inside_encoder(pp_rank - 1) and \
                (not parallel_state.is_inside_decoder(pp_rank - 1)) and parallel_state.is_inside_decoder()
    ...
    def __call__(self, tokens, position_ids, attention_mask):
        ...
        if num_image_tokens > 0:
            ...
            # If there are image tokens and this stage receives full embeddings,
            # make sure we compensate for expansion of image tokens.
            # Note that this will set a recv_buffer_seq_length for the encoder stage,
            ...
            if self._recv_only_vision_embeds:
                ...
                recv_buffer_seq_length = min(self._num_img_embeddings + num_tokens - num_image_tokens,
