...
def model_provider(
    ...
    Args:
        ...
        add_encoder (bool): Construct the encoder module (used with pipeline parallelism). Defaults to True.
            When we use pipelining, the encoder will live on only a subset of the pipeline stages 
            (specifically, only the first stage).
    ...
    # seq_length and encoder_seq_length denote the vision model sequence length.
    # Override if the user provided something else.
    ...
    if torch.distributed.get_rank() == 0 and old_seq_length != args.seq_length:
        warnings.warn(
            f"Changed seq_length and encoder_seq_length (vision model sequence length) from "
            f"{old_seq_length} to num_image_tokens ({num_image_embeddings})"
        )
    ...
    if mp_padding_needed > 0:
        if language_transformer_layer_spec.submodules.self_attention.params.get('attn_mask_type', '') == \
            AttnMaskType.causal:
            language_transformer_layer_spec.submodules.self_attention.params['attn_mask_type'] = \
                AttnMaskType.padding_causal
        elif language_transformer_layer_spec.submodules.self_attention.params.get('attn_mask_type', '') == \
...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    Returns:
        train_ds, val_ds, test_ds (megatron.core.datasets.multimodal_dataset.MockMultimodalDataset):
            Train, validation, and test datasets, respectively.
    """
    args = get_args()
...
def get_batch(data_iterator):
    ...
    if cp_size > 1 or args.sequence_parallel:
        ...
        if mp_padding_needed_for_text > 0:
            tokens, position_ids, labels, loss_mask = [torch.nn.functional.pad(item, (0, mp_padding_needed_for_text))
                                                       for item in (tokens, position_ids, labels, loss_mask)]
        packed_seq_params = context_parallel.get_packed_seq_params(tokens, img_seq_len, mp_padding_needed_for_text,
                                                                   cp_size, args.use_packed_sequence)
    ...
    # Use the attention mask type defined in layer spec.
    # Typically no mask for the vision model and causal mask for the text model.
    attention_mask = None  
...
def add_vlm_extra_args(parser):
    ...
    group.add_argument("--dataloader-seq-length", type=int,
