AIAK_TRAINING_PATH="${AIAK_TRAINING_PATH:-$(cd "$(dirname "$0")/../../.." && pwd)}"
...
_TMP_SUFFIX=${SLURM_JOB_ID:-$$}
mkdir -p ./tmp/
SAVE_LANGUAGE_MODEL=./tmp/language-mcore-${_TMP_SUFFIX}
SAVE_VISION_MODEL=./tmp/vision-model-mcore-${_TMP_SUFFIX}
SAVE_ADAPTER=./tmp/adapter-mcore-${_TMP_SUFFIX}
SAVE_PATCH=./tmp/patch-mcore-${_TMP_SUFFIX}
