...
class Partition(object):
    ...
    def process_json_file(self, file_name):
        ...
        proc_start = time.time()
        total_bytes_processed = 0
        total_sentence_lens = {key: 0 for key in self.args.json_keys}
        ...
        for i, (doc, sentence_lens, bytes_processed) in enumerate(encoded_docs, start=1):
            ...
            for key in doc.keys():
                ...
                total_sentence_lens[key] += sum(sentence_lens[key])

            self.print_processing_stats(i, proc_start, total_bytes_processed)

        fin.close()
        builders[key].finalize(output_idx_files[key])
        print(f"{input_file_name} total tokens: {total_sentence_lens}")
...
def get_args():
    ...
    group.add_argument('--tokenizer-type', type=str, required=True,
                       choices=[
                           'NullTokenizer',
                           'Llama2Tokenizer', # megatron verison
    ...
    group.add_argument('--seq-length', type=int, default=None, help='sequence length')
    ...
    args.training_phase=constants.TrainingPhase.PRETRAIN
