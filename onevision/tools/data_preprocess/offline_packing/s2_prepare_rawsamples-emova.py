...
# ### 参数配置
target_directory = "/mnt/cluster"   # 最终数据存放的位置

# current_file = Path(__file__).resolve()
# target_directory = current_file.parent

newDir = "raw_packing_data_emova_3000tk"                   # 转 webdataset 之前数据的存放位置 (jpg, json)
SRC_DIR_IMGS = "/vlm/xiangan/datasets/aiak_caption_emova_300k/"    # 原始 img  数据的存放位置
SRC_DIR_JSONS = "/vlm/xiangan/datasets/aiak_caption_emova_300k/"   # 原始 json 数据的存放位置
SRC_DST_EXTENSIONS = ("jpg", "json")
# f_toklens_originalsample = os.path.join(target_directory, "token_info_v2_emova_3000tk.txt")
# s1 的结果
f_toklens_originalsample = "/workspace/gits/train_rice_vl/tools/data_preprocess/offline_packing/token_info_v2_emova_3000tk.txt"
PACKED_LENGTH = 8192
# s2 的结果位置
...
n_packed_samples=100  # 测试用，输出几条打包后的数据
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS, thread_name_prefix="PackThread") as executor:
        ...
        if f_TEST:
            futures = {
                ...
                for s1, idx_knapsack in enumerate(idx_knapsacks[0:n_packed_samples])
            }
        else:
            futures = {
                executor.submit(process_knapsack, s1, idx_knapsack, dst_dir): s1
                for s1, idx_knapsack in enumerate(idx_knapsacks)
            }
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                completed += 1
                # 每完成1%输出一次进度
                if completed % (max(1, total_knapsacks // 10)) == 0:
                    print(f"已完成 {completed}/{total_knapsacks} 组数据 ({completed/total_knapsacks*100:.1f}%)")
            except Exception as e:
                print(f"处理第 {s1} 组数据时发生错误: {str(e)}")
    
    print(f"所有数据处理完成，共处理 {completed} 组")
